...
$t_sql = " SELECT COUNT(1) FROM `recipes` $where ";
...
<div class="container">
  ...
  <?php
  if (empty($rows)) {
    include __DIR__ . '/list-no-data.php';
  } else {
    include __DIR__ . '/list-content-admin.php';

    // if (isset($_SESSION['admin'])) {
    //   include __DIR__ . '/list-content-admin.php';
    // } else {
    //   include __DIR__ . '/list-content-no-admin.php';
    // }
  }
  ?>
</div>
...
<script>
  ...
  const deleteOne = recipe_id => {
    // question: 1. 若要在詢問時呈現名字? 2. 點選後在詢問時整列要呈現明顯的底色
    if (confirm(`確定要刪除編號為 ${recipe_id} 的資料嗎?`)) {
      location.href = `del.php?recipe_id=${recipe_id}`;
    }
  }
</script>
